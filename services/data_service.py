    ...
    def _initialize_seed_data(self) -> None:
        ...
        for s in delhi_stations:
            ...
            record = DataRecord(
                ...
                category="Delhi Ingress",
                payload={
                    "station_id": s["station_id"],
                    "location": s["location"],
                    "pollutants": {"pm2_5": s["pm2_5"], "pm10": s["pm10"], "no2": s["no2"], "o3": s.get("o3", 38.0), "so2": s.get("so2", 14.0), "co": s.get("co", 2.1), "aqi": s["current_aqi"]},
                    "meteorology": {"temp_c": s["temperature_c"], "humidity_pct": s["humidity_pct"], "wind_speed_kmh": s["wind_speed_kmh"], "pressure_hpa": s.get("pressure_hpa", 1014.0), "rainfall_mm": s.get("rainfall_mm", 0.0), "pbl_height_m": s["pbl_height_m"]}
                },
